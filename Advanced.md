...
```bash
keksbox@rehkakeks:~ $ sudo nmcli dev wifi hotspot ifname wlan0 ssid KeksboxAP password "kb"
```

Edit the NetworkManager configuration to assign a static IP to the AP:
```bash
keksbox@rehkakeks:~ $ sudo nano /etc/NetworkManager/system-connections/KeksboxAP.nmconnection
```
Add or modify the following lines under the `[ipv4]` section:
```
[ipv4]
method=shared
